...
@interface XTThirdViewController ()<UITableViewDelegate,UITableViewDataSource,XTThirdTableViewCellDelegate>
@property (nonatomic,strong)UITableView *tableView;
/** 数据源数组 */
@property (nonatomic, strong) NSMutableArray *array;
@end

@implementation XTThirdViewController
...
- (void)viewDidLoad {
    ...
    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc]initWithImage:[UIImage imageNamed:@"发布(1)"] style:1 target:self action:@selector(publish)];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    XTThirdTableViewCell *cell = [XTThirdTableViewCell cellWithTableView:tableView];
    cell.selectionStyle = UITableViewCellSelectionStyleNone;
    cell.delegate = self;
    Comment *comment = self.array[indexPath.section];
...
- (void)thirdTableViewCellsearcimage:(UIButton *)button{
    ...
    Comment *comment = self.array[path.section];
...
- (void)publish{
    ...
    UIAlertController *vc = [UIAlertController alertControllerWithTitle:@"发帖" message:@"添加图片,文字来发帖 和钓友 一起交流 心得(功能持续开发中...)" preferredStyle:1];
...
- (void)thirdTableViewCellbutton:(UIButton *)button{
    
    if (button.tag == 1000) {
        UIAlertController *vc = [UIAlertController alertControllerWithTitle:@"点赞成功" message:@"" preferredStyle:1];
        ...
        UIAlertController *vc = [UIAlertController alertControllerWithTitle:@"成功踩掉" message:@"" preferredStyle:1];
        ...
        UIAlertController *vc = [UIAlertController alertControllerWithTitle:@"评论成功" message:@"" preferredStyle:1];
